...
var SITE_VIDEO = function () {
	...
	var play = function () {
		...
		if(data.video_type == 'vimeo'){
			...
			tv = new YT.Player('img_' + code, {videoId: data.video_id, playerVars: options, events: {'onReady': onPlayerReady}});
	...
	// loadPlaylist 는 mute playerVar 로 건 음소거를 해제하고, loadVideoById 는 loop 컨텍스트를 무너뜨린다.
	// 영상은 생성자의 videoId 로 싣고, 반복은 playerVars 의 loop + playlist 조합이 처리한다.
