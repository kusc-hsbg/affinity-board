...
var SITE_SHOP_CART = function () {
    ...
    return {
        ...
        // MFE 주문 재시도 시 이전 선택을 누적하지 않고 현재 목록으로 교체한다.
        setSelectedCartItems: function (item_codes) {
            selectedCartItem = item_codes.slice();
        },
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.msg === 'SUCCESS') {
                    ...
                    // 댓글은 위에서 화면을 갱신한다. 목록 조회에는 후기 원문 코드만 보낸다.
                    if (type === 'post') {
    ...
    var switchToBlockButton = function (login_member_code, review_code, review_member_code, type) {
        $('._review_button._block_' + review_member_code)
            ...
            .on('click', function (e) {
                e.stopPropagation();
                if (type === 'comment') {
                    SITE_SHOP_REVIEW.addBlockComment(login_member_code, review_code);
                } else {
