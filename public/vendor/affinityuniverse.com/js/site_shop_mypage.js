var SITE_SHOP_MYPAGE = function(){
  ...
  var getOrderList = function(type, is_c3 = false){
    ...
    // 주문처리 현황에서 특정 칸을 누른 경우 그 상태로만 목록을 좁힌다
    var summaryStatus = urlParams.get('summary-status') || '';
    // 집계 기간과 목록 기간을 맞추기 위해 위젯이 심어둔 개월 수를 함께 보낸다
    var summaryMonths = $('#shop_mypage_order_summary').data('months') || '';

    $.ajax({
      type : 'POST',
      data : {'page' : currentPage, 'type' : type, 'isMigration' : isMigration, 'summary_status' : summaryStatus, 'summary_months' : summaryMonths},
